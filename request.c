...
void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg) {
   ...
   sprintf(body, "<html><title>networks Error</title>");
   sprintf(body, "%s<body bgcolor=""fffff"">\r\n", body);
   sprintf(body, "%s%s: %s\r\n", body, errnum, shortmsg);
   sprintf(body, "%s<p>%s: %s\r\n", body, longmsg, cause);
   sprintf(body, "%s<hr>networks Web Server\r\n", body);
...
void requestServeDynamic(int fd, char *filename, char *cgiargs) {
   ...
   sprintf(buf, "%sServer: networks Web Server\r\n", buf);
...
void requestHandle(int fd, thread *worker) {
   ...
   if (strcasecmp(method, "GET")) {
      requestError(fd, method, "501", "Not Implemented", "networks Server does not implement this method");
      return;
   }
   requestReadhdrs(&rio);
   ...
   if (is_static) {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
         requestError(fd, filename, "403", "Forbidden", "networks Server could not read this file");
         return;
      }
      worker->statics++;
      ...
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
         requestError(fd, filename, "403", "Forbidden", "networks Server could not run this CGI program");
         return;
      }
      worker->dynamics++;
